...
std::string STTriangleMesh::LoadObj(std::vector<STTriangleMesh*>& output_meshes, const std::string& filename){
    ...
    for(unsigned int mesh_id=0; mesh_id<shapes.size(); mesh_id++)
    {
        ...
        static std::map<std::string, STTexture*> allTex;
		...
		if(mesh.material_ids[0]>=0){
            ...
            //updated: cached texture STTextures
            if (allTex.find(colorMap) != allTex.end()) {
                stmesh->mSurfaceColorTex = allTex.find(colorMap)->second;
                ...
                if (colorMap != "") {
                    ...
                    allTex[colorMap] = stmesh->mSurfaceColorTex;//new STTexture(stmesh->mSurfaceColorImg,STTexture::kNone);
                }
            }
